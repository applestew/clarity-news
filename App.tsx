...
const App: React.FC = () => {
    ...
    const loadNews = useCallback(async (
        ...
        // Pass current titles to Gemini to avoid repetition
        const currentTitles = articles.map(a => a.title);
        const fetchedArticles = await fetchCuratedNews(profile, scope, query, region, currentTitles);

        if (fetchedArticles.length > 0) {
            setArticles(prevArticles => {
                // Combine and filter for unique titles
                const combined = isAutoRefresh ? [...fetchedArticles, ...prevArticles] : fetchedArticles;
                const unique = combined.filter(
                    (item, index, self) =>
                        index === self.findIndex((t) => t.title === item.title)
                );
                return unique.slice(0, 15); // Keep a reasonable number of articles
            });
    ...
    useEffect(() => {
        if (!userProfile || currentView !== ViewState.FEED) return;
        
        const interval = setInterval(() => {
            console.log("Auto-refreshing news...");
            loadNews(userProfile, filterScope, searchQuery, regionFilter, true);
        }, 180000); // 3 minutes
        
        return () => clearInterval(interval);
    }, [userProfile, filterScope, searchQuery, regionFilter, currentView, loadNews]);
    ...
    const handleFilterChange = (scope: FilterScope) => {
        ...
        setArticles([]); // Force clear to show loading and fresh data
    ...
    const handleRefresh = () => {
        if (userProfile) {
            setArticles([]); // Clear existing state to force fresh feel
